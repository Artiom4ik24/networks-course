...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - 0x62cbf4b4. Оно связывает запросы и ответы между DHCP-клиентом и сервером
...
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   - До завершения DHCP-обмена используется 0.0.0.0 в качестве исходного и 255.255.255.255 в качестве конечного (широковещательная рассылка)
...
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - Чтобы неактивные клиенты не занимали IP-адреса и сервер мог эффективно ими управлять
   - 59 минут, 59 секунд
