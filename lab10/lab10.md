...
2. Запустите pingplotter, выберите имя какого-либо целевого хоста (например, c).
...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   - 20 байт![alt text](image-2.png)
   - 8 + 28 = 36 байт![alt text](image-3.png)
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Version, Header Length, Explicit Congestion Notification, Total Length, Fragment Offset, Protocol, Source Address, Destination Address
      - Version, Header Length, Total Length, Fragment Offset, Protocol, Source Address, Destination Address
      - Identification, Time to Live, Header Checksum
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Оно скачет в диапазоне примернно 8000 - 60000
      - Я пытался найти, но так и не нашёл. Оно как будто просто рандомное
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Identification меняется, Time to Live - нет
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - По какой-то причине трассировка с 3500 байтами не работает: вывод команды только * (см. скриншот) ![alt text](image-8.png)
      Я попробовал поставить размер 1500 и не увидел фрагментации
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Только Identification и Header Checksum
