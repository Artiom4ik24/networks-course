...
def handle_client(client_socket, client_address, sem):
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Использование: python server_limited.py <concurrency_level>")
        sys.exit(1)

    try:
        concurrency_level = int(sys.argv[1])
    except ValueError:
        print("Ошибка: concurrency_level должен быть целым числом.")
        sys.exit(1)

    sem = threading.Semaphore(concurrency_level)
