...
Картинка (браузер автоматически её скачивает):
...
#### Решение
Итак, для начала нам нужно выяснить, за какое время сформируется первый пакет. 56 байт = 448 бит, поэтому это произойдёт за $\frac{448}{128000}=0,0035$ секунд. После этого информация начнёт выгружаться на канал связи. Последний бит информации попадёт на канал через $\frac{448}{10^6} = 0,000448$ секунд. Ему останется только пройти по каналу, то есть потратить $0,005$ секунд. Итого всего потребуется $0,008948$ секунд, т.е. примерно $9$ мс. 
