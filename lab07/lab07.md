...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - 0
   - Так как установлен только SYN-флаг
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - 0
   - 1
   - Он прочитал порядковый номер от клиента и прибавил к нему 1
   - Присутствуют оба флага: SYN и ACK
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - 1, 631, 13339, 40167, 85351, 93823
   - 2.847, 2.848, 2.993, 3.125, 3.125, 3.258
   - 2.99, 2.99, 3.125, 3.26, 3.301, 3.390
   - 0.15, 0.15, 0.133, 0.135, 0.175, 0.132
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Около 41 кбит/с
   - 684 байт + 60 байт = 744 байт (пакет и ответ на него) было передано за 0.145233 секунд. Поэтому, пропусная способность = 5123 байт в секунду, т.е. примерно 41 кбит/с
